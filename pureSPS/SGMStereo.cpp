...
SGMStereo::SGMStereo() : disparityTotal_(SGMSTEREO_DEFAULT_DISPARITY_TOTAL),  //256
disparityFactor_(SGMSTEREO_DEFAULT_DISPARITY_FACTOR),  //256
sobelCapValue_(SGMSTEREO_DEFAULT_SOBEL_CAP_VALUE),  //15
censusWindowRadius_(SGMSTEREO_DEFAULT_CENSUS_WINDOW_RADIUS),   //2
censusWeightFactor_(SGMSTEREO_DEFAULT_CENSUS_WEIGHT_FACTOR),    //1.0/6.0
aggregationWindowRadius_(SGMSTEREO_DEFAULT_AGGREGATION_WINDOW_RADIUS),   //2
smoothnessPenaltySmall_(SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_SMALL),  //100
smoothnessPenaltyLarge_(SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_LARGE),   //1600
consistencyThreshold_(SGMSTEREO_DEFAULT_CONSISTENCY_THRESHOLD) {}   //1
...
void SGMStereo::compute(const IplImage* leftImage,            //sps中转到sgm的入口
	const IplImage* rightImage,
	float* disparityImage)
{
	initialize(leftImage, rightImage);

	computeCostImage(leftImage, rightImage);  //计算代价图
...
void SGMStereo::initialize(const IplImage* leftImage, const IplImage* rightImage) {
	setImageSize(leftImage, rightImage);   //计算得到步幅，且是16的倍数
	allocateDataBuffer();
}

void SGMStereo::setImageSize(const IplImage* leftImage, const IplImage* rightImage) {
	width_ = static_cast<int>(leftImage->width);
	height_ = static_cast<int>(leftImage->height);
	if (rightImage->width != width_ || rightImage->height != height_) {
		throw std::invalid_argument("[SGMStereo::setImageSize] sizes of left and right images are different");
	}
	widthStep_ = width_ + 15 - (width_ - 1) % 16;  //计算步幅（一定是16的倍数）
}

void SGMStereo::allocateDataBuffer() {
	leftCostImage_ = reinterpret_cast<unsigned short*>(_mm_malloc(width_*height_*disparityTotal_*sizeof(unsigned short), 16));  //左代价图
	rightCostImage_ = reinterpret_cast<unsigned short*>(_mm_malloc(width_*height_*disparityTotal_*sizeof(unsigned short), 16));  //右代价图

	int pixelwiseCostRowBufferSize = width_*disparityTotal_;   //逐行像素的缓冲大小
	int rowAggregatedCostBufferSize = width_*disparityTotal_*(aggregationWindowRadius_ * 2 + 2);   //行并集代价缓冲大小
...
void SGMStereo::computeCostImage(const IplImage* leftImage, const IplImage* rightImage) {
	...
	convertToGrayscale(leftImage, rightImage, leftGrayscaleImage, rightGrayscaleImage);  //将左右转为灰度图

	memset(leftCostImage_, 0, width_*height_*disparityTotal_*sizeof(unsigned short));
	computeLeftCostImage(leftGrayscaleImage, rightGrayscaleImage);  //计算做边代价图
...
void SGMStereo::convertToGrayscale(const IplImage* leftImage,
	const IplImage* rightImage,
